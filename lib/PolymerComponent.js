    ...
    c.bindToElement = (element, output, context) => {
      ...
      outports.forEach((outport) => {
        ...
        if (outport === 'event') {
          ...
          c.originalFire = element.dispatchEvent.bind(element);
          const el = element;
          el.dispatchEvent = (event) => {
            if (outports.indexOf(event.type) !== -1) {
              // If an event has a dedicated port, then it should not be bound here
              c.originalFire(event);
              return;
            }
            if (event.type.indexOf('-changed') !== -1) {
              // Polymer 1.x fires an event for each changed property. Ignore these
              c.originalFire(event);
              return;
            }
            const value = (event.detail && event.detail.value) ? event.detail.value : event.detail;
            output.send({
              event: {
                action: event.type,
                payload: value,
      ...
      c.element.dispatchEvent(new CustomEvent('noflo:ready', {
        detail: true,
      }));
    };
    ...
    c.process((input, output, context) => {
      ...
      inports.forEach((inport) => {
        ...
        if (Array.isArray(c.element[inport])) {
          ...
          c.element.set(inport, packets);
          return;
        }
        activated = true;
        c.element.set(inport, input.getData(inport));
      });
